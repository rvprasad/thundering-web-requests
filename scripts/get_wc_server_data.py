...
def get_data(impl, nums, reqs):
    ...
    for i in range(1, 6):
        ...
        acc = []
        file_name = f'data/master10/{impl}/wc-{nums}-{reqs}-{i}-server.log'
        try:
...
with open(f'data/wc-server-perf.txt', 'wt') as f:
...
with open('data/wc-server-max-serviced-reqs.csv', 'wt') as f:
...
with open('data/wc-server-min-serviced-reqs.csv', 'wt') as f:
    print("# X@Y should be interpreted as X requests in Y MBps "
          "configuration", file=f)
    print("Impl,100@2,500@2,1000@2,1500@2,2500@2,100@6,500@6,1000@6,"
          "1500@6,2500@6,100@10,500@10,1000@10,1500@10,2500@10", file=f)
